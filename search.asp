...
if (isNaN(wrds)) {tps=1} // Поиск в публикациях (2 - объявления, 3 - сайты)
var sstr=""
var ssql=""
var qsql=""
var  tsql=""
...
// объявления	
	...
	if (sch!="") {
		...
		ssql=wsql+" UNION "+wsql2+" UNION "+wsql3+" order by 2"
	}}
              ...
              <tr bgcolor="#FBF8D7"> 
                <td height="35" bgcolor="#FFFFFF" bordercolor="#FFFFFF" valign="middle"> 
                  <p class="menu02"><img src="images/e06.gif" width="16" height="9" alt="" border="0"> 
                    <a href="index.asp">72RUS.RU</a> / Поиск <%if (tps==2) {%> в объявлениях<%}%><%if (tps==1) {%> в публикациях<%}%><%if (tps==3) {%> в каталоге<%}%></p>
                </td>
              </tr>
            </table>
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr> 
                        <td bgcolor="#FFFFFF" align="center"> 
                          <p> 
                            <input type="text" name="sch" size="40" value="<%=sch%>" style="BACKGROUND-COLOR: #FFFFFF; BORDER-BOTTOM: #000000 1px solid; BORDER-LEFT: #000000 1px solid; BORDER-RIGHT: #000000 1px solid; BORDER-TOP: #000000 1px solid; COLOR: #303030; FONT-FAMILY: tahoma; FONT-SIZE: 11px; WIDTH: 250px" >
...
<input type="hidden" name="tps" value="<%=tps%>">
                          ...
                          <p class="digest"><a href="searchall.asp?sch=<%=sch%>&wrds=<%=wrds%>&sensation=<%=sens%>" class="digest">Везде</a> 
                            | 
                            <%if (tps!=2) {%>
                            <a href="search.asp?sch=<%=sch%>&wrds=<%=wrds%>&sensation=<%=sens%>&tps=2" class="digest">В 
                            объявлениях</a> | 
                            <%}%>
                            <%if (tps==2) {%>
                            В объявлениях  | 
                            <%}%>
                            <%if (tps!=3) {%>
                            <a href="search.asp?sch=<%=sch%>&wrds=<%=wrds%>&sensation=<%=sens%>&tps=3" class="digest">В 
                            каталоге сайтов</a> | 
                            <%}%><%if (tps==3) {%> В каталоге сайтов | <%}%>
                            <%if (tps!=1) {%>
                            <a href="search.asp?sch=<%=sch%>&wrds=<%=wrds%>&sensation=<%=sens%>&tps=1" class="digest">В 
                            публикациях</a> 
                            <%}%><%if (tps==1) {%> В публикациях<%}%>
                            | 
