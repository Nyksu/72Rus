...
if (!isFirst) {
	...
	if (news.length>30000) {ErrorMsg+="Слишком длинный текст публикации.<br>"}
	if (news.length<0) {ErrorMsg+="Слишком короткий текст публикации.<br>"}
...
<html>
<head>
<title>Добавление публикации. <%=tit%></title>
...
<form name="form1" method="post" action="addpub.asp">
  <p align="center">Для добавления публикации, пожалуйста, заполните поля формы: 
...
<%}%>
<hr width="780">
<p align="center">index.aspВернуться на главную страницу</p>
